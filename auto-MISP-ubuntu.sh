...
openssl req -new -newkey rsa:4096 -days 365 -nodes -x509 -subj "/C=AU/ST=Queensland/L=Brisbane/O=Project/OU=Security/CN=`hostname`" -keyout misp.key  -out misp.crt &>> $logfile
...
if [ -f /etc/apache2/sites-available/misp-ssl.conf ]; then
	...
	echo "<VirtualHost ubuntu.projectmisp.com:443>" >> /etc/apache2/sites-available/misp-ssl.conf
	echo "	ServerAdmin admin@projectmisp.com" >> /etc/apache2/sites-available/misp-ssl.conf
	echo "	ServerName ubuntu.projectmisp.com" >> /etc/apache2/sites-available/misp-ssl.conf	
